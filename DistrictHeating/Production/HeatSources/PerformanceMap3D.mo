...
model PerformanceMap3D "Heat source based on data from a 3D performance map"
  //Extensions
  extends DistrictHeating.Production.BaseClasses.PartialHeatSource(modulating=
        true);
  ...
  IDEAS.Utilities.Tables.InterpolationTable3D interpolationTable(space=space)
    "Interpolation table to determine the efficiency at a modulation grade"
    annotation (Placement(transformation(extent={{-10,12},{10,32}})));
  IDEAS.Utilities.Tables.InterpolationTable3D interpolationTableQMax(space=
        space)
